...
export class ModalRef<T> {
   ...
   * modal options
   */
  private readonly _modalOptions: ModalOptions;
  ...
  get shouldCloseOnNavigating(): boolean {
    const {closeOnNavigating} = this._modalOptions;

    return ValidationUtil.isDefined(closeOnNavigating) ? closeOnNavigating! : true;
  }
...
export interface ModalOptions {
  ...
  /**
   * set `false` to keep modal on navigating
   */
